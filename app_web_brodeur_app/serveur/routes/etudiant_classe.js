...
router.get("/", async (req, res) => {
  try {
    const resultat = await client.query("SELECT * FROM etudiant_classe");
    ...
    res
      .status(500)
      .json({ message: "Erreur lors du fetch des etudiant classe" });
  }
});
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const resultat = await client.query(
      "SELECT * FROM etudiant_classe WHERE etudiant_id_etudiant = $1",
      [id]
    );

    if (resultat.rowCount === 0) {
      logger.error(`Aucune classe d'etudiant n'a le id : ${id}`);
      return res
        .status(404)
        .json({ message: `Aucune classe d'etudiant n'a le id :${id}` });
    }
