...
-- Create the sequence
CREATE OR REPLACE FUNCTION utilisateur_id_utilisateur_trg() RETURNS TRIGGER AS $$ BEGIN -- Check if the id_utilisateur is NULL and assign a new value from the sequence
    IF NEW.id_utilisateur IS NULL THEN NEW.id_utilisateur := nextval('utilisateur_id_utilisateur_seq');
END IF;
