...
CREATE TABLE niveau_performance (
    ...
    critere_id_critere INT NOT NULL,
...
CREATE TABLE session_utilisateur (
    ...
    tentatives_echoues         INT,
    date_derniere_tentative    DATE,
    ip_derniere_connexion      INT,
...
CREATE TABLE utilisateur (
    ...
    type_utilisateur         CHAR(1) NOT NULL,
    professeur_id_professeur INT,
    etudiant_id_etudiant     INT
);

-- Séquences pour PostgreSQL
CREATE SEQUENCE utilisateur_id_utilisateur_seq START 1;

-- Triggers pour PostgreSQL
CREATE OR REPLACE FUNCTION utilisateur_id_utilisateur_trigger()
RETURNS TRIGGER AS $$
BEGIN
...
FOR EACH ROW EXECUTE FUNCTION utilisateur_id_utilisateur_trigger();

-- Contraintes de clés étrangères pour PostgreSQL
...
ALTER TABLE classe
    ADD CONSTRAINT classe_cours_fk
        FOREIGN KEY (cours_id_cours, cours_session_id_session)
            REFERENCES cours (id_cours, session_id_session);
